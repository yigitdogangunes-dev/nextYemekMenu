      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-6 relative z-10">
        ...
        <div className="flex bg-white/40 dark:bg-white/[0.05] p-1.5 rounded-2xl backdrop-blur-md border border-white/20 dark:border-white/10">
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-xl font-rajdhani font-bold text-lg transition-all ${activeTab === "foods" ? "bg-teal-500 text-white shadow-md" : "text-gray-600 dark:text-gray-300 hover:bg-white/20"
              }`}
          >
            Yemekler
          </button>
            ...
            >
              <div className="flex justify-between items-center bg-white/30 dark:bg-white/[0.02] p-4 rounded-3xl border border-white/20 dark:border-white/10 backdrop-blur-md">
                ...
                <button
                  ...
                  className="bg-teal-500 hover:bg-teal-600 text-white px-6 py-3 rounded-2xl font-rajdhani font-bold text-lg uppercase tracking-wider transition-all shadow-apple hover:shadow-apple-hover active:scale-95"
                >
                  + Yeni Ekle
                </button>
              </div>
                    ...
                    <div className="bg-white/60 dark:bg-black/40 backdrop-blur-xl border border-white/30 dark:border-white/10 rounded-3xl p-6 shadow-xl mb-6">
                      ...
                      <form onSubmit={handleCreateFood} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <input
                          ...
                          className="w-full bg-white/50 dark:bg-black/50 border border-gray-200 dark:border-white/10 rounded-2xl px-5 py-4 font-rajdhani text-lg focus:outline-none focus:ring-2 focus:ring-teal-500/50 text-gray-900 dark:text-white"
                        />
                        <div className="relative">
                          <input
                            ...
                            className="w-full bg-white/50 dark:bg-black/50 border border-gray-200 dark:border-white/10 rounded-2xl px-5 py-4 pr-12 font-rajdhani text-lg focus:outline-none focus:ring-2 focus:ring-teal-500/50 text-gray-900 dark:text-white"
                          />
                        ...
                        <div className="md:col-span-2 lg:col-span-4 flex justify-end gap-3 mt-2">
                          ...
                          <button type="submit" className="bg-teal-500 text-white px-8 py-3 rounded-2xl font-rajdhani font-bold text-lg shadow-md hover:shadow-lg transition-all">Kaydet</button>
                        </div>
                      </form>
                    </div>
                  </motion.div>
              ...
              <AnimatePresence>
                {editingFood && (
                  ...
                  >
                    <div className="bg-teal-500/10 dark:bg-teal-500/5 border border-teal-500/30 rounded-3xl p-6 shadow-xl relative backdrop-blur-xl">
                      <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-teal-400 to-teal-600 rounded-t-3xl" />
                      <h3 className="font-rajdhani font-bold text-xl mb-4 text-teal-800 dark:text-teal-400 flex items-center gap-2">
                      ...
                      <form onSubmit={handleEditFood} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        <input
                          ...
                          className="w-full bg-white/50 dark:bg-black/50 border border-teal-500/20 rounded-2xl px-5 py-4 font-rajdhani text-lg focus:outline-none focus:ring-2 focus:ring-teal-500/50 text-gray-900 dark:text-white"
                        />
                        <div className="relative">
                          <input
                            type="number" placeholder="Fiyat" required min="0"
                            value={editingFood.price} onChange={(e) => setEditingFood({ ...editingFood, price: e.target.value })}
                            className="w-full bg-white/50 dark:bg-black/50 border border-teal-500/20 rounded-2xl px-5 py-4 pr-12 font-rajdhani text-lg focus:outline-none focus:ring-2 focus:ring-teal-500/50 text-gray-900 dark:text-white"
                          />
                          <span className="absolute right-5 top-1/2 -translate-y-1/2 font-bold text-gray-400">₺</span>
                        </div>
                        <select
                        ...
                        <div className="md:col-span-2 lg:col-span-4 flex justify-end gap-3 mt-2">
                          ...
                          <button type="submit" className="bg-teal-500 text-white px-8 py-3 rounded-2xl font-rajdhani font-bold text-lg shadow-md hover:shadow-lg transition-all">Güncelle</button>
                        </div>
                      </form>
                    </div>
                  </motion.div>
              ...
              <div className="space-y-8">
                {[
                  ...
                  { key: "cold", title: "🥗 Soğuklar & Salatalar" },
                  { key: "drink", title: "🥤 İçecekler" },
                  { key: "dessert", title: "🍮 Tatlılar " }
                ].map(category => {
                  const categoryFoods = foods.filter(f => f.category === category.key);
                  if (categoryFoods.length === 0) return null;

                  return (
                    <div key={category.key} className="space-y-4">
                      <h4 className="font-bebas text-2xl text-gray-700 dark:text-gray-300 tracking-widest pl-3 border-l-4 border-teal-500">
                        {category.title}
                      </h4>
                      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                        {categoryFoods.map(f => (
                          <div key={f._id} className={`flex items-center justify-between p-4 bg-white/60 dark:bg-black/40 backdrop-blur-xl border ${f.status === 'passive' ? 'border-red-500/30 dark:border-red-500/20 opacity-75 grayscale-[0.3]' : 'border-white/30 dark:border-white/10'} rounded-3xl shadow-sm transition-all gap-4`}>

                            <div className="flex items-center gap-4">
                              <div className="w-12 h-12 rounded-xl bg-gray-100 dark:bg-white/5 flex items-center justify-center text-2xl flex-shrink-0">
                                {f.category === "soup" ? "🥣" : f.category === "mainCourse" ? "🍛" : f.category === "side" ? "🍚" : f.category === "cold" ? "🥗" : f.category === "drink" ? "🥤" : "🍮"}
                              </div>
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  <span className="text-teal-600 dark:text-teal-400 font-bold text-sm bg-teal-50 dark:bg-teal-500/10 px-2 py-0.5 rounded-lg">
                                    {f.price} ₺
                                  </span>
                                  {f.status === "passive" && (
                            ...
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                className="p-2 bg-teal-50 text-teal-600 hover:bg-teal-500 hover:text-white dark:bg-teal-500/10 dark:text-teal-400 dark:hover:bg-teal-500 dark:hover:text-white rounded-xl transition-all"
                              ...
                              <button
                                ...
                                className={`relative inline-flex h-7 w-12 items-center rounded-full transition-colors ${f.status === "active" ? "bg-teal-500" : "bg-gray-300 dark:bg-gray-600"}`}
